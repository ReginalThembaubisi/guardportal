...
import type { PropertyResponse, UnitResponse } from "../../api/types";
...
export default function AdminUnitsPage() {
  ...
  return (
    <Layout title="Admin — Units">
      {error && <p className="error">{error}</p>}

      {properties && properties.length === 0 && <p className="empty">No properties yet — create one first.</p>}
      ...
      {properties && properties.length > 0 && (
        <>
          ...
          <form onSubmit={submit}>
            <label>
              Unit number
              <input type="text" value={unitNumber} onChange={(e) => setUnitNumber(e.target.value)} placeholder="e.g. 42" required />
            </label>
